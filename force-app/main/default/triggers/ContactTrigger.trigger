...
trigger ContactTrigger on Contact(before insert, after insert, before update) {
	...
	if (Trigger.isInsert && Trigger.isBefore) {
		ContactTriggerHelper.processInsertedContacts(Trigger.new); // if DummyJSON_Id__c is null, generate a random number between 0 and 100 and set this as the contact's DummyJSON_Id__c value
	} else if (Trigger.isInsert && Trigger.isAfter) {
		ContactTriggerHelper.updateNewContacts(Trigger.new); // if DummyJSON_Id__c is less than or equal to 100, call the getDummyJSONUserFromId API
	} 
